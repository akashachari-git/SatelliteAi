...
# Mount static files to serve generated previews, overlays, and reports
...
def login_with_google(req: GoogleLoginRequest, response: Response):
    """
    Verifies genuine Google ID token with Google's public tokeninfo endpoint,
    identifies the user by Google's permanent sub ID, and creates a secure session.
    """
...
def get_current_user(
    ...
    """Returns the verified user profile for the current active session, or 401."""
...
def logout_user(
    ...
    """Destroys current server session and clears authentication cookie."""
    session_token = request.cookies.get(SESSION_COOKIE_NAME)
    if not session_token and authorization and authorization.startswith("Bearer "):
        session_token = authorization.replace("Bearer ", "").strip()
    if not session_token and x_session_id:
...
async def upload_image(file: UploadFile = File(...)):
    """Uploads GeoTIFF/TIFF/PNG/JPG, extracts metadata, creates web preview."""
    ...
    # Validate that image is legitimate satellite / Earth Observation imagery
    is_satellite, reason, conf = SatelliteImageValidator.validate_satellite_image(str(file_path), metadata)
    if not is_satellite:
        # Delete rejected non-satellite upload immediately
        try:
            if file_path.exists():
                file_path.unlink()
        except Exception:
...
def validate_images(req: ValidateRequest):
    """Validates compatibility and co-registration across 1 or 2 images."""
    if len(req.image_paths) == 0:
        raise HTTPException(status_code=400, detail="No images provided.")

    # Validate each image through SatelliteImageValidator first
...
def analyze_query(
    ...
    """Main Agentic Controller entrypoint executing 13-stage orchestration."""
    if not req.query.strip():
        raise HTTPException(status_code=400, detail="Query cannot be empty.")
    if len(req.image_paths) == 0:
        raise HTTPException(status_code=400, detail="At least one image is required.")

    # Defensively resolve relative or uploaded paths
    resolved_paths = []
    for p in req.image_paths:
        ...
        if path_obj.is_absolute() and path_obj.exists():
            ...
            resolved_paths.append(str((BASE_DIR / p).resolve()))
...
def validate_gemini_key(body: Dict[str, str] = Body(...)):
    ...
    try:
        ...
        # Fallback check against models catalog
        try:
...
def generate_pdf(req: ReportRequest):
    """Generates formal PDF report using ReportLab."""
...
def generate_json(req: ReportRequest):
    """Generates structured JSON intelligence export."""
    json_url = ReportService.generate_json_export(req.analysis_data)
    return {"json_url": json_url}

@app.get("/api/tools")
def list_tools():
    """Lists all registered tools in the Specialist Tool Registry."""
    return {"tools": tool_registry.list_tools()}

@app.get("/api/models")
def list_models():
    """Returns model information, BigEarthNet adaptation metadata."""
...
def get_demo_scenarios():
    """Returns catalog of pre-configured demo scenarios."""
...
def load_demo_scenario(scenario_id: str):
    """Prepares and loads images and metadata for a chosen demo scenario."""
    scenarios = DemoDataGenerator.ensure_demo_files()
...
def list_benchmarks():
    """Lists supported benchmark datasets and configuration status."""
...
def run_benchmark(benchmark_key: str):
    """Executes evaluation on benchmark test split."""
