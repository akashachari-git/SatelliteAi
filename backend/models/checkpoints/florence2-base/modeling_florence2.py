...
class Florence2Decoder(Florence2LanguagePreTrainedModel):
    ...
    def forward(
        ...
        if past_key_values is not None:
            ...
            elif isinstance(past_key_values, (tuple, list)) and len(past_key_values) > 0 and past_key_values[0] is not None and getattr(past_key_values[0][0], "shape", None) is not None:
...
class Florence2PreTrainedModel(PreTrainedModel):
    ...
    def _supports_sdpa(self):
        ...
        """
        lm = getattr(self, "language_model", None)
        return getattr(lm, "_supports_sdpa", False) if lm is not None else False
